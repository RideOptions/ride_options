...
import 'package:rideoptions/Component/Services/ride_service.dart';
import 'package:rideoptions/Component/common_function.dart';
  ...
  Widget build(BuildContext context) {
    ...
    return UpgradeAlert(
      child: Scaffold(
        ...
        //         rideService.listenToRideRequests(uid);
        //         setState(() {});
        //       });
        ...
        //                         ElevatedButton(
        //                           onPressed: () {
        //                             rideService.getRideRequestAsync(uid);
        //                           },
        //                           child: Text("Fuction Call"),
        //                         ),
        //                       ],
        //                     ),
        //             ),
        ...
        body: SafeArea(
          child: StreamBuilder<List<RideRequestModel>>(
            stream: rideService.getRidesRequest(uid),
            builder: (BuildContext context,
                AsyncSnapshot<List<RideRequestModel>> snapshot) {
              if (snapshot.connectionState == ConnectionState.waiting) {
                return SizedBox
                    .shrink(); // Loading indicator while waiting for data
              } else if (snapshot.hasError) {
                return Text('Error: ${snapshot.error}');
              } else {
                List<RideRequestModel> rideRequests = snapshot.data ?? [];

                if (rideRequests.isEmpty) {
                  // Return a default widget when there is no data
                  return Center(
                      ...
                      return Container(
                          ...
                          child: Column(
                            ...
                            children: [
                              ...
                              Text(
                                rideRequests[index].sourceLocation?.location ??
                                    "",
                                style: blackTextLightIn14px(),
                              ),
                              SizedBox(
